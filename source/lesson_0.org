...
# #+EXPORT_FILE_NAME: exports/{{{input-file}}}
#+HTML_HEAD: <style>
...
Once you have Homebrew installed, you'll need to install python! There are also instructions on how to install software using Homebrew on that page - but we aren't looking to install ~wget~, we want ~python3~! Give ~brew edit wget~ a try.
